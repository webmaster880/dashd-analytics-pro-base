...
### Negative Values And Data Quality Warnings

Отрицательные значения raw data не блокируются, потому что некоторые текущие данные могут требовать подтверждения у data owner.
Плагин обрабатывает их как soft warning:

- в админке Raw Data строки с `value < 0` подсвечиваются и получают бейдж `Needs review`;
- в ручном вводе raw data отрицательное значение сохраняется, но показывает предупреждение;
- на frontend-графиках отрицательные точки/столбцы получают warning-обводку и tooltip;
...
При экспорте PDF легенда графика создается отдельным временным canvas-блоком из видимых серий.
Этот блок не зависит от интерактивной или canvas-легенды Chart.js, поэтому сохраняет
цветные плашки даже если обычная легенда скрыта на странице или не попадает в `html2canvas`.
